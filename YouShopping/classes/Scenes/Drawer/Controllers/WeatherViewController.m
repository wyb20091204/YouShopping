...
- (void)viewDidLoad {
    ...
    UIImageView *imageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"wback"]];
    ...
    [self.zTableView registerNib:[UINib nibWithNibName:@"ZCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"zcell"];
    
    [self.wTableView registerNib:[UINib nibWithNibName:@"WCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"wcell"];
    //设置指数表格的cell的高度
...
- (BOOL)navigationShouldPopOnBackButton {
    ...
    if (self.time.text.length == 18) {
        NSRange range = {14, 3};
        NSString *string = [self.time.text substringWithRange:range];
        self.block(self.cityLoca, string);
    } else {
        NSRange range = {14, 2};
        NSString *string = [self.time.text substringWithRange:range];
        self.block(self.cityLoca, string);
